...
- **MIDIテイク自動保存 (v0.9.1)**: **ドラレコ方式のMIDIキャプチャ**。MIDI入力を常時監視し、
  最初のノートオンでテイク開始→**10秒無入力(鍵盤保持なし)で自動クローズ→
  `<exe横>\MIDI\rec_YYYYMMDD_HHMMSS.mid` に保存**(120bpm/960tpq固定グリッド)。
  アーム/停止操作なし=弾いたものは失われない。ノートオン3個未満の誤タッチテイクは破棄。
  **Open MIDI file... も同じ `MIDI\` フォルダが起点=弾く→自動保存→バウンスが1フォルダで一周**。
